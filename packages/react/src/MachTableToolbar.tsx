...
function button(
  ...
  return createElement("button", {
    key: label,
    type: "button",
    className: "mach-toolbar__button",
    title: label,
    "aria-label": label,
    disabled,
    onClick
...
function renderMain<TData>(props: MachTableToolbarProps<TData>): ReactNode[] {
  ...
  if (enabled(props.features, "search")) {
    main.push(createElement("label", { className: "mach-toolbar__search", key: "search" },
  ...
  if (enabled(props.features, "refresh")) {
    main.push(button("刷新", props.loading ? "…" : "↻", () => {
      props.onRefresh?.();
      if (!props.onRefresh) {
  ...
  if (enabled(props.features, "columns")) {
    main.push(button("列设置", "☷", (event) => {
      if (props.commands) props.commands.openColumns(event.currentTarget);
      else props.api?.columns.openWorkbench(event.currentTarget);
    }));
  }
...
function renderAside<TData>(props: MachTableToolbarProps<TData>): ReactNode[] {
  ...
  if ((props.selectedCount ?? 0) > 0) {
    aside.push(createElement("button", {
      key: "selection",
      type: "button",
      className: "mach-toolbar__selection",
      onClick: () => props.onClearSelection?.()
    }, `已选 ${props.selectedCount} 项 ×`));
  }
  if (enabled(props.features, "undoRedo")) {
    aside.push(button("撤销", "↶", () => {
  ...
  if (enabled(props.features, "export")) {
    aside.push(button("导出 CSV", "⇩", () => {
      props.commands?.exportCsv(props.exportFilename);
    }, !props.commands));
  }
  if (enabled(props.features, "fullscreen")) {
    aside.push(button("全屏", "⛶", () => {
      void props.commands?.toggleFullscreen();
    }, !props.commands));
  }
