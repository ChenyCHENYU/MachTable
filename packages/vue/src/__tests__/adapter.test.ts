...
import {
  useMachTableEditing,
  type UseMachTableEditingReturn,
...
import {
  ...
  type UseMachTableControllerReturn,
...
describe("Vue configuration helpers", () => {
  it("keeps page props intact while contextually typing row events", () => {
    const config = defineVueTableConfig<{ id: number }>({
      rowKey: "id",
      stripedRows: true,
      onRowClicked: (event) => event.rowNode.data?.id,
    });

    expect(config.rowKey).toBe("id");
    expect(config.stripedRows).toBe(true);
  });
});
...
describe("Vue adapter", () => {
  ...
  it("applies plugin defaults and lets component props override them", async () => {
    ...
    await nextTick();
    expect(
      host.querySelector(".mach-root")?.classList.contains("mach-theme-dark"),
    ).toBe(false);
    expect(
      (host.querySelector(".mach-pagination") as HTMLElement).style.display,
    ).toBe("none");
    app.unmount();
  });
  ...
  it("runs application event observers and page listeners exactly once", async () => {
    ...
    app.use(MachTablePlugin, {
      defaults: { onCellClicked: applicationObserver },
    });
    app.mount(host);
    await nextTick();
  ...
  it("loads a dedicated config with named presets and exposes option provenance", async () => {
    ...
    expect(resolved.pagination).toEqual({
      pageSize: 50,
      pageSizeOptions: [20, 50],
    });
    expect(resolved.defaultColDef).toEqual({ sortable: false, editable: true });
    expect(table.value.explainOption("size")).toEqual(
      expect.objectContaining({ source: "table props", value: "large" }),
    );
    expect(table.value.explainOption("editType")).toEqual(
      expect.objectContaining({ source: "preset:editable" }),
    );
    app.unmount();
  });
  ...
  it("merges scoped config, reports invalid presets and explains built-in values", () => {
    ...
    expect(inheritedWarning).toHaveBeenCalledWith(
      expect.objectContaining({ code: "UNKNOWN_PRESET", preset: "missing" }),
    );
  ...
  it("accepts datasource and forwards ordinary host attributes", async () => {
    ...
    expect(host.querySelector("#orders-grid")?.getAttribute("aria-label")).toBe(
      "orders",
    );
    app.unmount();
  });
  ...
  it("reactively updates theme and row data", async () => {
    ...
    expect(
      host.querySelectorAll(".mach-row[data-index]").length,
  ...
  it("renders typed row-first Vue column functions and refreshes their VNodes", async () => {
    interface Row {
      id: number;
      status: "pending" | "done";
    }
    const rows = ref<Row[]>([{ id: 1, status: "pending" }]);
    const columns = defineVueColumns<Row>([
      {
        field: "status",
        render: (row, index, params) =>
          h(
            "span",
            { class: `render-status render-status--${row.status}` },
            `${index}:${params.value}`,
          ),
      },
    ]);
    ...
    app.mount(host);
    await nextTick();
    expect(host.querySelector(".render-status")?.textContent).toBe("0:pending");

    rows.value = [{ id: 1, status: "done" }];
    await nextTick();
    expect(host.querySelector(".render-status--done")?.textContent).toBe(
      "0:done",
    );
    app.unmount();
  });
      ...
      defineComponent({
        setup: () => () =>
          h(
            ...
            {
              "cell-name": (props: any) =>
                h("strong", { class: "slot-cell" }, props.value),
              "header-name": () =>
                h("span", { class: "slot-header" }, "业务名称"),
              "editor-name": (props: any) =>
                h("input", {
                  class: "slot-editor",
                  value: props.value,
                  onInput: (event: Event) =>
  ...
  it("provides a reactive save, partial-success and rollback workflow", async () => {
    ...
    let editing: UseMachTableEditingReturn<{
      id: string;
      name: string;
    }> | null = null;
    const app = createApp(
    ...
    for (let rowIndex = 0; rowIndex < 2; rowIndex++) {
      ...
      (host.querySelector(".mach-editor-input") as HTMLInputElement).value =
        `changed-${rowIndex}`;
  ...
  it("guards editing failures, concurrent saves and before-unload cleanup", async () => {
    ...
    const api = {
      ...
      editing: {
        getDirtyRowIds: () => ["1"],
        getChanges: () => [
          { rowId: "1", colId: "name", oldValue: "before", newValue: "after" },
        ],
        save: saveRequest,
        rollback: vi.fn(() => true),
        markSaved: vi.fn(),
    ...
    await expect(editing!.save(saveChanges as any)).rejects.toThrow(
      /already in progress/,
    );
  ...
  it("cancels stale remote queries and ignores late responses", async () => {
    ...
    const request = vi.fn(
      (params: any) =>
        new Promise<{ rows: { id: string }[]; total: number }>((resolve) => {
  ...
  it("drives server pagination and preserves selected rows across pages", async () => {
    ...
    let remote: UseMachTableQueryReturn<{ id: string; name: string }> | null =
      null;
    const host = document.createElement("div");
    const app = createApp(
      defineComponent({
        setup() {
          remote = useMachTableQuery({
            ...
            request: (params) =>
              new Promise((resolve) =>
                requests.push({ page: params.page, resolve }),
              ),
          });
          return () => h("div");
        },
    ...
    await Promise.resolve();
    await nextTick();
    remote!.bindings.value.onSelectionChanged?.({
      selectedRows: remote!.rows.value,
    } as any);
    expect(remote!.selectedKeys.value).toEqual(["1"]);

    remote!.bindings.value.onPaginationChanged?.({
      page: 2,
      pageSize: 20,
    } as any);
    ...
    expect(remote!.selectedRows.value.map((row) => row.name)).toEqual([
      "one",
      "two",
    ]);
    app.unmount();
  ...
  it("represents select-all matching as compact server rules", async () => {
    ...
    expect(remote!.selectionState.value).toEqual({
      mode: "allMatching",
      excludedKeys: [],
    });
  ...
  it("surfaces remote validation failures and supports explicit retry and abort", async () => {
    ...
    const request = vi.fn(
      (params: any): Promise<{ rows: { id: string }[]; total: number }> =>
        requests.shift()!(params) as Promise<{
          rows: { id: string }[];
          total: number;
        }>,
    );
    ...
    await remote!.reload();
    expect(remote!.error.value).toEqual(
      expect.objectContaining({ message: "offline" }),
    );
    const errorOverlay = remote!.bindings.value
      .overlayErrorTemplate as () => string;
  ...
  it("handles server sort, filter, page selection and controlled selected keys", async () => {
    ...
    remote!.bindings.value.onSortChanged?.({
    ...
    expect(request.mock.lastCall?.[0]).toEqual(
      expect.objectContaining({ quickFilterText: "from-grid" }),
    );
    ...
    expect(request.mock.lastCall?.[0]).toEqual(
      expect.objectContaining({ quickFilterText: null }),
    );
    app.unmount();
  });
  ...
  it("supports manual query mode without hidden network requests", async () => {
    ...
    const request = vi.fn(
      async ({ query: criteria }: { query: { keyword: string } }) => ({
    ...
    remote!.bindings.value.onFilterChanged?.({
      ...
      advancedFilterModel: {
        ...
        root: {
          ...
          filter: {
            type: "text",
            conditions: [{ match: "equals", value: "after" }],
          },
        },
      },
    } as any);
  ...
  it("wires the optional toolbar to commands, API fallbacks, events and slots", async () => {
    ...
    const densityTrigger = host.querySelector<HTMLButtonElement>(
      '.mach-toolbar__select-control [role="combobox"]',
    )!;
    expect(density.hidden).toBe(true);
    densityTrigger.click();
    expect(
      document.body.querySelector('.mach-select-listbox[role="listbox"]'),
    ).toBeTruthy();
    densityTrigger.click();
  ...
  it("composes local and remote Vue controllers with one command surface", async () => {
    let local: UseMachTableControllerReturn<{
      id: string;
      name: string;
    }> | null = null;
    const externalLoading = ref(false);
    const theme = ref<"light" | "dark">("light");
    const editMode = ref<"none" | "cell" | "row">("none");
    const host = document.createElement("div");
    document.body.appendChild(host);
    const app = createApp(
      defineComponent({
    ...
    await nextTick();
    expect(local!.table.api.value?.getOption("theme")).toBe("dark");
    expect(local!.table.api.value?.getOption("editType")).toBe("cell");
    expect(local!.table.api.value?.getOption("editableIndicator")).toBe("hover");
    expect(local!.table.api.value?.getOption("loading")).toBe(true);
