...
| 分组行复选框 | 整组级联选中（跳过禁选行） |
| 空格键（焦点在分组行） | 展开/收起 |
| `api.hierarchy.setAllGroupsExpanded(true/false)` | 全部展开/收起 |
...
| 能力 | 行为 |
| --- | --- |
| 展开/收起 | ▶ 点击或 `api.hierarchy.setRowExpanded(id, true/false)`；`setAllDetailsExpanded(true)` 全展开 |
| 级联勾选 | 勾选父行自动勾选全部子孙；父复选框三态（全选/半选/空） |
| 过滤 | 命中子树自动保留其祖先链（不会出现"孤儿行"） |
| 排序 | 仅同级兄弟间排序，不破坏层级 |
| 事务 | `api.rows.transact({ remove: [parentNode] })` 递归删除整棵子树 |
| 序号 | 只计实际数据行 |
...
api.hierarchy.isRowExpanded("1-2");
api.rows.getById("1-2-1");          // 任意深度节点
api.rows.forEach((n, i) => ...);    // 深度优先遍历全部节点
```
