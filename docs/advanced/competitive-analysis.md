...
| --- | --- | --- | --- |
| MachTable 0.25 | DOM 行列双虚拟化；复杂业务浏览和录入；Vue 优先、React 官方支持 | 从 0.9.1 起源码公开可查看，使用前需书面授权 | 领域 API、严格分层配置、区段隔离持久化、冲突保存、随机块调度、Worker 边界与零 Core 运行时依赖 |
...
| 服务端分组行模型 | 0.25 已完成有界随机访问块调度，但尚无 AG SSRM 的分组/聚合 Store | 先冻结服务端 group key、child count、aggregate 与事务协议；保持为可选 Feature |
...
- 形成 `0.10 → … → 0.25 → 1.0` 路线图；0.25 已完成高频业务、治理、性能与正式接入前 API 收口，1.0 继续由真实项目证据驱动。
...
0.10—0.25 的高频业务、治理、性能和 API 收口路线已经完成。下一步先让真实项目验证随机块、Worker、编辑与状态闭环，再按证据建设高级过滤构建器或服务端分组协议；Pivot、公式、图表继续按独立 ADR 决策。这样既扩展企业适用面，又不会丢掉 MachTable 小内核、低依赖和复杂业务编辑的差异化优势。
