...
| --- | --- | --- | --- |
| MachTable 0.18 | DOM 行列双虚拟化；复杂业务浏览和录入；Vue 优先、React 官方支持 | 从 0.9.1 起源码公开可查看，使用前需书面授权 | 分层配置、嵌套过滤、冲突保存、命名视图、可诊断 Feature/性能、原地框架 renderer、零 Core 运行时依赖 |
...
| 随机访问块缓存 | 超长滚动、任意跳转仍弱于 AG Infinite Model | 独立 `RemoteBlockStore`，有界 LRU、并发/乱序/重试协议 |
...
- 形成 `0.10 → … → 0.18 → 1.0` 路线图；0.18 前高频业务与治理里程碑已经合并验收，1.0 继续由真实项目证据驱动。
...
MachTable 已经不是“普通 Vue 表格”的水平：基础网格、双虚拟化、复杂编辑、列工作台、懒加载树、主从、嵌套过滤、冲突保存、状态治理和跨框架内核已经形成完整产品骨架。与成熟商业网格相比，主要差距不在再加几个按钮，而在 **随机访问服务端 Row Model、分析插件、真实无障碍/公开对比证据和生态成熟度**。

0.10—0.18 的高频业务与治理路线已经完成。下一步应先让真实项目验证这些闭环，再按证据建设随机块远程模型和高级过滤构建器；Pivot、公式、图表继续按独立 ADR 决策。这样既扩展企业适用面，又不会丢掉 MachTable 小内核、低依赖和复杂业务编辑的差异化优势。
