...
| --- | --- | --- | --- |
| MachTable 0.28 | DOM 行列双虚拟化；复杂业务浏览和录入；Vue 优先、React 官方支持 | 从 0.9.1 起源码公开可查看，使用前需书面授权 | 领域 API、严格分层配置、零复杂度豁免、区段隔离持久化、冲突保存、随机块调度、Worker 边界与零 Core 运行时依赖 |
...
| 服务端分组/Pivot/块缓存 | 平面块缓存有；分组/Pivot 无 | 强（商业 SSRM） | 待测 | 无公开证据 | 服务端数据，深度待测 | 商业 | Pivot 强，服务端协议待测 |
| 单元格编辑 | 强 | 强 | 强 | 强 | 强 | 强 | 有 |
...
| 缺口 | 业务影响 | 建议交付物 | 验收指标 |
| --- | --- | --- | --- |
| 服务端数据契约仍缺层级 Store | 大数据分组、聚合、权限过滤无法形成统一后端协议 | 基于现有平面块模型定义可选分组 Store 协议 | 乱序响应不覆盖新状态；分组刷新/事务/聚合有故障注入 E2E |
...
| 批量修改审阅面板 | 已有详细失败/冲突协议，下一步提供可选审阅 UI | 基于 `api.editing.save()` 与 `failedRowIds`，不内置业务合并策略 |
| 服务端分组行模型 | 0.28 已有有界随机访问块调度，但尚无 AG SSRM 的分组/聚合 Store | 先冻结服务端 group key、child count、aggregate 与事务协议；保持为可选 Feature |
...
- Worker 排序/过滤已通过可选 Processor 实现并保留同步小数据快路径；Pivot 仍不进入 Core。
...
- 形成 `0.10 → … → 0.25 → 0.28 → 1.0-rc` 路线图；0.25 完成 API 收口，0.28 清零复杂度债务并进入真实项目验证，1.0 继续由证据驱动。
...
| 0.20—0.25 | API/资源/发布治理 | 冻结领域门面、增量管线、请求优先级与并发、共享 Observer、区段隔离持久化、500 列/生命周期/发布产物门禁完成 |
| 0.28 | 稳定性与复杂度治理 | 公共 API 快照无漂移，31 项历史复杂度豁免清零，文档和发布基线统一后进入真实项目试用 |
...
0.10—0.25 的高频业务、治理、性能和 API 收口路线已经完成，0.28 又完成全量复杂度治理。下一步先让真实项目验证随机块、Worker、编辑与状态闭环，再按证据建设高级筛选构建器或服务端分组协议；Pivot、公式、图表继续按独立 ADR 决策。这样既扩展企业适用面，又不会丢掉 MachTable 小内核、低依赖和复杂业务编辑的差异化优势。
