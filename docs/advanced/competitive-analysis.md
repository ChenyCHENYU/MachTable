...
| --- | --- | --- | --- |
| MachTable 0.15 | DOM 行列双虚拟化；复杂业务浏览和录入；Vue 优先、React 官方支持 | 从 0.9.1 起源码公开可查看，使用前需书面授权 | 分层配置、远程查询、业务编辑、组合控制器、可选工具栏、可恢复列宽与自动状态、列工作台、懒加载树、可选 XLSX、零 Core 运行时依赖 |
...
- 形成 `0.10 → 0.11 → 0.12 → 0.13 → 1.0` 路线图；0.13 前各里程碑已经合并验收，1.0 继续由真实项目证据驱动。
...
MachTable 已经不是“普通 Vue 表格”的水平：基础网格、双虚拟化、复杂编辑、列工作台、懒加载树、主从、状态治理和跨框架内核已经形成完整产品骨架。与成熟商业网格相比，主要差距不在再加几个按钮，而在 **随机访问服务端 Row Model、高级筛选、分析插件、真实无障碍/性能证据和生态成熟度**。

0.10—0.13 的高频业务路线已经完成。下一步应先让真实项目验证这些闭环，再建设随机块远程模型与高级筛选；Pivot、公式、图表继续按独立 ADR 决策。这样既扩展企业适用面，又不会丢掉 MachTable 小内核、低依赖和复杂业务编辑的差异化优势。
