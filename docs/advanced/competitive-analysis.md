...
| --- | --- | --- | --- |
| MachTable 0.23 | DOM 行列双虚拟化；复杂业务浏览和录入；Vue 优先、React 官方支持 | 从 0.9.1 起源码公开可查看，使用前需书面授权 | 分层配置、冲突保存、随机块并发调度、增量失效、Worker 边界、API/性能/产物治理、零 Core 运行时依赖 |
...
| 服务端分组行模型 | 0.23 已完成有界随机访问块调度，但尚无 AG SSRM 的分组/聚合 Store | 先冻结服务端 group key、child count、aggregate 与事务协议；保持为可选 Feature |
...
- 形成 `0.10 → … → 0.23 → 1.0` 路线图；0.23 前高频业务、治理与性能内核里程碑已经合并验收，1.0 继续由真实项目证据驱动。
...
| 0.20—0.23 | API/资源/发布治理 | 惰性领域、增量管线、请求优先级与并发、共享 Observer、500 列/生命周期/发布产物门禁完成 |
...
0.10—0.23 的高频业务、治理和性能内核路线已经完成。下一步先让真实项目验证随机块、Worker、编辑与状态闭环，再按证据建设高级过滤构建器或服务端分组协议；Pivot、公式、图表继续按独立 ADR 决策。这样既扩展企业适用面，又不会丢掉 MachTable 小内核、低依赖和复杂业务编辑的差异化优势。
